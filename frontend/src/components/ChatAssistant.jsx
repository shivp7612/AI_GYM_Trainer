...
export default function ChatAssistant({ setView }) {
  ...
  return (
    <div className="min-h-screen bg-dark text-white pb-6 px-4 md:px-8 relative overflow-hidden select-none flex flex-col items-center">
      <div className="absolute top-[-10%] left-[-10%] w-[40vw] h-[40vw] rounded-full bg-brand-primary/5 blur-[120px]"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[40vw] h-[40vw] rounded-full bg-brand-accent/5 blur-[120px]"></div>
      ...
      <div className="w-full max-w-3xl flex-1 flex flex-col pt-8 relative z-10 animate-fade-in-up">
        ...
        {/* CHAT CHASSIS */}
        <div className="flex-1 futuristic-card border border-gold/15 rounded-3xl overflow-hidden flex flex-col mb-4">
              ...
              >
                <div 
                  className={`max-w-[80%] rounded-2xl px-5 py-3.5 text-sm leading-relaxed ${
                    m.sender === 'user' 
                      ? 'neon-btn-magenta text-white rounded-tr-none' 
                      : 'bg-dark-card border border-gold/20 text-slate-200 rounded-tl-none'
                  }`}
                >
          ...
          {/* CHIPS SUGGESTIONS */}
          <div className="px-6 py-4 border-t border-white/5 bg-dark-card">
            ...
            <div className="flex flex-wrap gap-2">
              {suggestions.map((s, idx) => (
                <button
                  ...
                  disabled={sending}
                  className="px-3.5 py-2 bg-white/5 hover:bg-gold/15 border border-white/5 hover:border-gold/30 transition-all rounded-xl text-xs text-slate-300 hover:text-gold font-medium active:scale-95"
                >
                  {s}
                </button>
              ))}
          ...
          <div className="p-4 border-t border-white/5 flex gap-3 items-center">
            <input 
              ...
              disabled={sending}
              className="flex-1 bg-dark-card border border-gold/15 outline-none rounded-xl px-4 py-3.5 text-sm focus:border-gold/50 text-white placeholder-dark-muted font-medium transition-all"
            />
            <button
              onClick={() => handleSendMessage()}
              disabled={sending || !inputText.trim()}
              className="w-12 h-12 neon-btn-gold rounded-xl flex items-center justify-center text-black shadow-lg active:scale-95 transition-all flex-shrink-0 disabled:opacity-50 font-bold"
            >
